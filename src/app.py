...
# 1) Connect to the database using SQLAlchemy's create_engine function. We use MySQL because our database is built on MySQL.
...
# 2) Execute the SQL sentences to create your tables using SQLAlchemy's execute function
create_tables = """
...
# 3) Execute the SQL sentences to insert your data using SQLAlchemy's execute function

# We erase all data before adding it for this exercise to avoid the error of duplicate entries.
delete_data = """
...
for statement in delete_data.split(';'):
    statement = statement.strip()
    if statement:
        connection.execute(statement)
...
"""
for statement in introduce_data.split(';'):
    statement = statement.strip()
    if statement:
        connection.execute(statement)
